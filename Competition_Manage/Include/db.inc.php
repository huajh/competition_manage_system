...
class DBSQL{
	...
	public function __construct(){
		try {											//捕获连接错误并显示错误文件utf8_unicode_ci utf-8
			$conn = mysql_connect(ServerName,UserName,PassWord)or die("不能连接数据库");
			mysql_query("SET CHARSET 'utf-8'",$conn);
		}catch (Exception $e)
		{
			$msg = $e;
			include(ERRFILE);
		...
		try {											//捕获数据库选择错误并显示错误文件
			mysql_select_db(DBName,$conn)or die("不能打开数据库");
		}catch (Exception $e)
		{
			$msg = $e;
			include(ERRFILE);
	 ...
	 * 返回：数组
	 */
	public function getInfo_by_name($user_name,$name)
	{
		$sql = "SELECT * FROM " . $name . " WHERE user_Name =' $user_name'";
		$r = $this->select($sql);
		return $r[0];
	}	
	/**
